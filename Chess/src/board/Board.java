...
public class Board {
	...
	public void movePiece(PlayerField from, int pTo, int xTo, int yTo) {
		Piece toMove = from.getPiece();
		if(yTo == 0 && toMove.getClass() == Pawn.class) {
			toMove = new Queen(toMove.getPlayer());//TODO be able to choose
		}
		from.removePiece();
		playerboards[pTo].getPlayerFieldAt(xTo, yTo).addPiece(toMove);
	}
}
